...
inline bool winner(const int8_t *g, const int p){
...
inline bool full(const int8_t *g){
    for(int i=0;i<9;i++)
...
struct State {
    int8_t nextPlayer; // -1, 0, 1
    int8_t nextGrid; // 0-8 or -1 for any
    array<int8_t, 9> og; // -1, 0, 1
    array<int8_t, 81> g; // -1, 0, 1
};
...
tuple<int,int> main1(const State &s, int sim_timelimit){
    ...
    for(int sim=1;; sim++){
        ...
        if(treeStateWin[stateIndex]+treeStateDraw[stateIndex]+treeStateLoss[stateIndex] != 0 && !gameOver(treeState[stateIndex]) && 100*treeState.size() < sim && treeState.size() < 10e8){
        ...
        if(sim % 100000 == 0){
            cout << "SIM:" << sim << "\t" << treeState.size() << endl;
            cout << "tree sim length:" << path.size() << endl;
    ...
    for(auto y : treeChildren[0]){
        ...
        int w = treeStateWin[y] + 0.5*treeStateDraw[y];
